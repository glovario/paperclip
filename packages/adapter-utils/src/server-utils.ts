...
export function renderPaperclipWakePrompt(
  ...
  const lines = resumedSession
      ? [
        ...
        "",
        "Auth contract: all authenticated Paperclip API requests use `Authorization: Bearer $PAPERCLIP_API_KEY`. `PAPERCLIP_API_KEY` is the only auth token in your environment — no other `PAPERCLIP_*` env var (e.g. `PAPERCLIP_AGENT_JWT_SECRET`, `PAPERCLIP_RUN_ID`, `PAPERCLIP_AGENT_ID`) is a bearer credential, and using one will silently misattribute the call.",
        "",
